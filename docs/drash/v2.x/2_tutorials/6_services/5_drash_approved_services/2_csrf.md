...
tutorial. So, please make sure you copy YOUR `x-csrf-token` and not the one in
this tutorial. Also, this tutorial is using headers and not cookies, so the
below command will set headers and not cookies to make the `POST` request._
