...
- Each code block __adds__ code to the code block __before__ it (if any). Code is highlighted to show the code (or code comment) that is being __added__.
- The Node code blocks will show code that creates a `context` object. This `context` object contains the `url` and `method` fields required by chains. These fields are mentioned at [Concepts > Chains > What Data Does It Process?](/drash-v3.x/concepts/chains#what-data-does-it-process).
...
If you do not see the issue you are facing below, know that you can always get help in our [Discord server](https://discord.gg/RhYs6zQJ) (specifically the `#help` channel).
...
- Learn how to [handle errors](/drash-v3.x/modules/request-chain/error-handling) in a more robust manner