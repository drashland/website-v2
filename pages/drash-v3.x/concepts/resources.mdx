...
<Tab>
```typescript
import { Chain, Resource } from "http://esm.sh/drash/modules/RequestChain/mod.native.ts";
...
<Tab>
```javascript
const { Chain, Resource } = require("@drashland/drash/modules/RequestChain/mod.polyfill");
...
All chain modules (in the [Modules](../concepts/framework#modules) part of the codebase) export a `Resource` class. You must extend this class to create your resources. Otherwise, your application might not work as expected.
...
```typescript filename="MyResource.ts"
// Import path is shortened for brevity
import { Resource } from "../path/to/modules/RequestChain/mod.native.ts";
