...
// Drash imports
import * as WebAPIChain from "https://esm.sh/@drashland/drash@v3.0.0-beta.01/esm/standard/chains/webapi/mod.ts";

// Deno imports
import { serve } from "https://deno.land/std/http/server.ts";

class Home extends WebAPIChain.Resource {
  public paths = ["/"];
...
// Drash imports
import * as ContextChain from "@drashland/drash@v3.0.0-beta.01/esm/standard/chains/contextchain/mod";
...
class Home extends ContextChain.Resource {
  public paths = ["/"];
...
(async() => {
  const server = createServer((request, response) => {
    request.url = `http://${hostname}:${port}${request.url}`;
...
(async() => {
  const server = createServer((request, response) => {
    request.url = `${protocol}://${hostname}:${port}${request.url}`;
...
// Drash imports
const ContextChain = require("@drashland/drash@v3.0.0-beta.01/esm/standard/chains/contextchain/mod");
