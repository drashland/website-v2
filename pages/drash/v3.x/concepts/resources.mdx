...
We prefer to use classes when processing resources internally.
...
To give you a better idea of how Drash compares to a framework that uses `app.get()`, see the JavaScript code blocks below showing how GET requests can be handled in Express and Drash's `RequestChain`.
...
__Drash `RequestChain`__

The example below shows a `RequestChain.Resource` using a "context" object like in Koa.

<Tabs items={["Deno 1.35.x", "Node v16.x"]}>
<Tab>
```js
// Deno 1.35.x

import * as RequestHandler from "http://..../drash/modules/RequestChain/native.ts";
...
class MyResource extends RequestChain.Resource {
  public paths = ["/"];
  public GET(context) {
    context.response = new Response(context.request.method + " received!");
  }
}
...
const RequestChain = require("@drashland/drash/modules/RequestChain/polyfill");
...
The example below shows a `RequestChain.Resource` using Web APIs (`Request` and `Response`).
...
const server = http.createServer((req, res) => {
  ...
  // Convert to Web API Request object
...
You might notice (now or when you start working with resource classes) that the syntax is very strict. __This is intentional.__ We made resource classes follow a strict interface in hopes that it promotes <LinkExternal href="https://en.wikipedia.org/wiki/Separation_of_concerns">separation of concerns</LinkExternal>. For example, the name you give a resource class makes you think about what its `paths` will look like and what logic might be contained in its HTTP methods.

## The Core Resource

### TLDR

All resources are extended from the `AbstractResource` class located in Drash's core code. Extending `AbstractResource` allows all resources to share the same interface and default behavior &mdash; throwing a `501 Not Implemented` error for any HTTP request method not implemented. Throwing a `501 Not Implemented` error is based on <LinkExternal href="https://datatracker.ietf.org/doc/html/rfc7231#section-4.1">RFC 7231 Section 4.1</LinkExternal>.

### Detailed Explanation

Drash's core code contains the core resource class: `AbstractResource`. Its only purpose is to provide chains with a base resource class they can extend &mdash; giving them a single source of default behavior. Chains can deviate from the `AbstractResource` class, but all of them will extend the `AbstractResource` class to standardize their default behavior.

The `AbstractResource` class has implementations for all of the HTTP request methods. Those implementations throw a `501 Not Implemented` error (based on <LinkExternal href="https://datatracker.ietf.org/doc/html/rfc7231#section-4.1">RFC 7231 Section 4.1</LinkExternal>). The implementations are written like:

```ts filename="AbstractResource.ts"
...
Given the above code, if we have a `MyResource` class that extends `AbstractResource` like ...
...
All chain modules export a `Resource` class. You have to extend this class to create your resources. Otherwise, your application might not work as expected.

### Detailed Explanation

All chains implement their own base `Resource` class (extended from the `AbstractResource` class). Doing it this way allows us to:
...
When you use a chain, you'll extend its `Resource` class to create your resources. For example, the code below shows you how you'd extend the base `Resource` class from Drash's chains:

<Tabs items={["ContextChain", "WebAPIChain"]}>
<Tab>
```ts
import * as ContextChain from "...";

class MyResource extends ContextChain.Resource {
  public paths = ["/teas"];

  public GET(context: SomeContext): SomeContext {
...
<Tab>
```ts
import * as WebAPIChain from "...";

class MyResource extends WebAPIChain.Resource {
  public paths = ["/teas"];
...
Creating resource classes is explained more in-depth their respective chain's pages below:

- [Tutorials > ContextChain > Create Resources](../tutorials/context-chain/create-resources)
- [Tutorials > WebAPIChain > Create Resources](../tutorials/web-api-chain/create-resources)
...
export default class Home extends Chain.Resource {
  public paths = ["/"];
...
- Read about [errors](./errors) and when they're thrown
...
- Your server is the server object you create in your chosen runtime. For example, in Node v16.x it could look like:
  ...
  ```
- Your content is anything you return from the [resources](./resources) you create. For example, the below code shows a combination of text content being returned from 3 different resources:
  ```ts
